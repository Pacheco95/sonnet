---
name: setup-sonnet
description: Set up a Linux machine to build and test Sonnet - check the prerequisites, guide the installs, bootstrap vcpkg, configure, build, run the tests on Lavapipe and take an editor screenshot. Use when someone asks to set up, bootstrap or first build the project on Linux, or when a fresh build fails for a missing prerequisite - a CMake configure or vcpkg install error, missing X11 or Wayland headers, no C++23 compiler, no Vulkan 1.4 device, or a sanitizer or coverage preset that will not configure.
argument-hint: "[check | <linux preset>]"
allowed-tools: Bash(python3 tools/check_setup.py), Bash(ldconfig -p), Bash(vulkaninfo --summary), Bash(git -C * status --porcelain)
---

# Set up Sonnet on Linux

Take this machine from a fresh clone to a green test run and an editor screenshot. Linux only. For Windows or macOS, point to `docs/build.md` and stop.
...
Then run step 1 again. If something required is still missing because the user declined or could not install it, don't loop: stop and go to step 5 with what is missing and why.
...
End with a short summary: what was installed or changed on the machine and by whom, the preset and compiler, the test counts (and any flaky test), whether the screenshot was taken, and what is still missing or left as a warning. Then give the three commands to keep, configure, build and test, with the actual compiler, vcpkg root and Lavapipe manifest filled in, exactly as they worked here.
