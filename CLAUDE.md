...
- **Rendering**: vk-bootstrap creates, Vulkan-HPP RAII wrappers own, `vkb::destroy_*` is never called, RAII members are declared in reverse destruction order. The render graph is an engine module. Shaders are Slang only, compiled by `slangc` at build time through `sonnet_add_shaders` and at runtime in the editor for hot reload; Slang reflection is the only shader reflection source. Set 0 is the bindless set, set 1 is push descriptors, push constants carry what varies per pass. Reversed-Z, negative viewport height for the Y flip, counter-clockwise front faces. Opaque draws are culled in a compute pass and submitted indirectly, one call per run of draws sharing a pipeline and a mesh, with the object index in the command's `firstInstance` (ADR-0012); blended draws keep the direct path. Where the device lacks `drawIndirectCount` (MoltenVK), culling writes one slot per candidate with zero instances for the culled ones and the batches draw with plain `drawIndexedIndirect` (ADR-0014).
