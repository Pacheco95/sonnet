...
- Locations are compiled in at every level. `trace` and `debug` calls compile out of Release builds through `SPDLOG_ACTIVE_LEVEL`. The build maps the source root to a relative prefix for `__FILE__` and `std::source_location`, so file names are repository-relative on every platform and the strings in shipped binaries carry no build-machine paths; debug information is not remapped, so debuggers keep finding the sources.
