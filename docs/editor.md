...
1. Events arrive through `nativeEvent` (to ImGui) and `event` (mouse deltas for the camera, and the game's input while playing; the app itself handles quit and resize). While the viewport camera is looking with the right button held, `nativeEvent` keeps mouse motion from ImGui so its cursor stays over the viewport; button events still reach it. When the look ends, SDL returns the visible cursor to its position before the drag, and the editor sends that position to ImGui.
...
File, Export... cooks the open project and assembles a directory that runs on its own ([player.md](player.md#what-an-export-is)). The dialog asks for the target platform and the folder to write into, which defaults to `export/<platform>` under the project. "Current scene only" becomes available when a scene file is open. It starts the bundle in that scene and omits the project's other scenes; prefabs and assets still travel with it. The scene is read from disk, so the dialog warns when the open scene has unsaved changes. After a successful export it reports what it wrote and replaces OK and Cancel with Close; Enter or Escape closes it too. A failed export leaves the dialog open with its error and OK and Cancel.
...
| `--shading-term TERM` | One term of the forward shading instead of the final image: `final`, `albedo`, `normal`, `sun-direct`, `shadow-factor`, `ibl-diffuse`, `ibl-specular`, `brdf-lut` or `cascade`, the View > Shading term menu's names in any case |
| `--settle-frames N` | Frames drawn after the assets have loaded, before playing or capturing; 10 by default |

`sonnet_editor --help` prints the same list, from the table the parser reads, with the shading terms and the exit codes, and opens no window; `tools/check_docs.py` fails when a flag in that table is missing from this section. A project and one of the two screenshots are required, and any mistake in the arguments fails the run rather than starting a different one. The run waits until the viewport has a target and `AssetDatabase::loading` is false, giving up after two minutes, then draws the settle frames, plays, selects, draws three more frames so the panels lay out what the selection shows, and captures both images from one frame. With `--play`, those three frames play too. Meanwhile the editor steps at a fixed 1/60 s instead of the wall clock, so `--play 3` is 180 steps however fast the machine draws. Before playing it seeds the scripts' `math.random` with a fixed value, which Lua otherwise seeds differently in every process, so a scene whose scripts draw random numbers, like the playground's spawner, plays the same too: the same arguments give the same image run after run, byte for byte. Five runs of the playground with `--play 3`, and three with `--play 10` and six crates on the multi-threaded physics, each wrote one image. The exit code is 0 once both files are written and 1 on any failure, which the log says. The viewport image is the render target's bytes, already display-encoded by the tone mapper, so it matches the screen; the window image is the swapchain image the frame presented, which needs `ISwapchain::readable` (every desktop driver, Lavapipe's headless surface and MoltenVK have it). Both are opaque: the scene's alpha is not coverage.
