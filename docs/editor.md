...
3. `render(commands, swapchainImage)` first polls the picker, then resets the graph, imports the viewport target and declares the forward pass, the id pass into a transient id image, the outline pass over the colour and the pick readback, then the ImGui pass into the swapchain image (cleared, with the viewport colour declared sampled) when there is an image, and executes the graph. It then records the frame's statistics.
