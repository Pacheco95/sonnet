...
|---|---|---|
| Shader draw parameters | 1.1 | Slang lowers `SV_VertexID` through `gl_BaseVertex`, and `SV_StartInstanceLocation` through `gl_BaseInstance` |
...
| Draw indirect count, multi-draw indirect, indirect first instance | 1.2 and 1.0 features | GPU-driven submission ([ADR-0012](decisions/0012-gpu-driven-rendering.md)) |
...
- Push constants: what varies per pass rather than per draw. Today only the shadow cascade; the draw's object index arrives in the vertex shader as `SV_StartInstanceLocation`, and the mesh's vertex buffer address lives in that object's entry of the object array ([ADR-0012](decisions/0012-gpu-driven-rendering.md)).

Per-object data lives in storage buffers addressed by index, so a scene pass records one `drawIndexedIndirectCount` per batch and the GPU supplies the draws.
...
Opaque draws are submitted by the GPU ([ADR-0012](decisions/0012-gpu-driven-rendering.md)). The CPU no longer records a call per draw; it records a call per *batch*, where a batch is a run of draws sharing a pipeline and a mesh. The draw list is already sorted by double-sidedness and then by mesh, so those runs are simply its segments.

- **Candidates.** Once per frame the renderer writes a `CullDraw` per entry of an order list into transient memory: the draw's world-space bounds, the object it draws, its submesh's index range, and which batch it belongs to. The bounds are the mesh's object-space box through the draw's transform, all eight corners, so a rotation stays conservative. There are two arrays: the opaque draws, which the cascades, the depth pre-pass and the forward pass all test, and every draw, which the editor's id and selection-mask passes test.
- **Culling.** `cull.slang` runs one thread per candidate. It rebuilds the frustum from the pass's view-projection matrix by the Gribb-Hartmann method, rejects a candidate whose bounds fall outside any plane, and appends a survivor's `IndirectCommand` to its batch's range of a device-local command buffer, taking its slot from an atomic add on that batch's counter. The command's `firstInstance` is the object index. One compute pass covers every scene pass of the frame: it clears each job's counters, then runs one dispatch per job, with barriers ordering the previous frame's command fetch before the writes and the writes before this frame's fetch. The selection-mask job also requires the candidate's `CullSelected` flag, so the mask holds exactly the selected silhouettes.
- **Submission.** Each drawing pass records, per batch, `bind pipeline, bind index buffer, drawIndexedIndirectCount` over that batch's command range and counter. An indirect draw cannot rebind the index buffer, which is why batches stop at a mesh boundary rather than collapsing to one call per pass.
- **What stays on the CPU.** Blended draws: their order is back to front and view-dependent, which is a sort rather than a filter. They take the same pipelines and the same shaders, passing the object index as the draw's first instance so nothing in the shader branches on how it was submitted. The per-frame object, material and light upload also stays on the CPU, and is the larger part of what a frame costs there.

`RendererSettings::jobs` spreads the per-frame fill — one `ObjectData` per draw, and the cull candidates per batch — over the job system ([ADR-0013](decisions/0013-job-system.md)); a null pool runs it on the calling thread, which is what the tests and `sonnet_cook` get. Every iteration writes one slot of its own, so the result does not depend on how it was split. It buys less than it looks like it should: the benchmark's fill goes from 0.26 ms to about 0.18 ms with one extra worker and then stops improving, because writing ten thousand 160-byte entries into `MemoryUsage::CpuToGpu` memory is bandwidth to host-visible device memory rather than computation. Threads cannot remove that; writing less can, by shrinking `ObjectData` or by not rewriting the objects that did not move, and that is where the next of this cost goes.
- **What this costs in visibility.** The CPU never learns how many draws survived: `RenderStatistics` reports what was submitted. The null device's trace shows `drawIndexedIndirectCount "draw commands" max N`, so the tests above `rhi` assert on the calls a pass makes, and the GPU tests on Lavapipe are what prove culling keeps the right things — a ground plane wider than the frustum and a lit box still render, pixel for pixel, when a crowd of draws outside the frustum is added to the scene.
