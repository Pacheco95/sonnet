...
- **Jolt** is suppressed wholesale by `race:JPH::` in `tools/tsan.supp`. The reports are on `TempAllocatorImpl`, whose plain `mTop` is written from several threads by design — Jolt's header says the ordering comes from job dependencies, and those resolve inside `JobSystemWithBarrier.cpp`, in the library. The suppression matches any frame, so it also stops the sanitizer policing how `physics` drives Jolt. Closing it is building Jolt with `-fsanitize=thread` for this preset alone. A triplet does that with less than the overlay port this entry first proposed, since the port itself needs no change: `triplets/x64-linux-tsan.cmake`, the `x64-linux` triplet plus `-fsanitize=thread` in `VCPKG_C_FLAGS` and `VCPKG_CXX_FLAGS` under `if(PORT STREQUAL "joltphysics")`, so no other library is rebuilt, with `linux-tsan` setting `VCPKG_TARGET_TRIPLET` to it and `VCPKG_OVERLAY_TRIPLETS` to `triplets/`. Jolt has to be compiled by the preset's own compiler (Clang 20 on the development machine), or its instrumentation and the test binary's runtime disagree; vcpkg does not pass `CC` and `CXX` through by default, so the triplet names them through `VCPKG_CHAINLOAD_TOOLCHAIN_FILE` or `VCPKG_ENV_PASSTHROUGH`, whichever the port's build honours, which is to be checked, not assumed. Then `race:JPH::` comes out of `tools/tsan.supp` and `physics_tests` runs under the sanitizer. What it reports then is either a real race in how `physics` drives Jolt, which gets fixed, or the one the file already names inside Jolt, `Mutex::try_lock` reading `mLockedThreadID` under `JPH_ENABLE_ASSERTS`, which gets a suppression naming that function alone. [build.md](build.md#presets) gains the triplet in the preset's row.
...
The second gap matters less than it looks, because `rhi` is main-thread-only by design, and that rule is now enforced rather than assumed: every device records the thread it was created on and asserts it on the frame bracket, resource creation and destruction, and the uploads and transient allocations ([rendering.md](rendering.md#frame-structure)). It catches the case the sanitizer is missing — a later change scheduling rhi work onto the pool — without needing the driver instrumented, and it found nothing to fix, which is the answer the assertion exists to keep true. What remains of this gap is only the instrumented Jolt, since an overlay port is the whole of the first item and the driver is not ours to instrument.

### A pending texture draws as nothing in particular

Decided: a texture still importing draws as a placeholder, and a mesh still importing draws nothing. ADR-0013's decision says the request form returns "a placeholder handle" and its consequences that every caller "has to be able to draw nothing for that asset"; this reads the first as the texture case and the second as the mesh case. If that reading counts as a change of direction rather than a clarification, a short ADR superseding the sentence comes with the change.

**Meshes: nothing, as today.** A stand-in mesh would have to be the right size to help, and nothing of the right size is to hand before the import: a unit box where a building goes misleads more than an empty space, and geometry that pops from a box into a mesh reads worse than geometry that appears. The entity is already whole meanwhile — in the hierarchy panel, selectable, with its transform — since a model is placed from its hierarchy ([Scene loading blocked the frame](#scene-loading-blocked-the-frame)). A box fitted to the glTF `POSITION` accessor's `min` and `max`, which the JSON carries and `importGltfStructure` could return, is the option to revisit if a user asks for one.

**Textures: a placeholder, which is not what happens today.** A texture is not requested at all where it matters most. `AssetDatabase::resolve`, which turns a material's source into the renderer's description, calls the synchronous `texture` for every slot, so the first frame that draws a file material imports and cooks its textures in place — the stall M8 removed for meshes, still there for a `.material.json` and its images. A glTF file's images are the exception, since they arrive in the same publish as its materials. What is shown for a texture that did not load is the renderer's fallback, white or a flat normal, the same whether it failed or has not arrived. The steps:

1. `resolve` asks with `requestTexture`, so a material is created at once and its textures import on the job system.
2. A texture still in flight resolves to a placeholder the database registers as a built-in beside the primitive meshes: a neutral grey for the base colour, so a pending surface reads as unfinished rather than as the material's untextured colour, and the renderer's existing white and flat-normal fallbacks for the other slots, which already mean "no effect". A texture that failed keeps today's fallback, so pending and failed look different. Telling them apart needs the database to say which a texture is: `m_pending` already knows a file texture in flight, and a glTF image is pending while its file is.
3. Publishing a texture refreshes the materials that read it. `refreshMaterials` does exactly that and is called only on a re-import today; the main-thread job that publishes a requested texture calls it too.
4. The test: a material file whose texture is not cooked yet resolves without touching the cache, reads the placeholder's index in its base-colour slot while `loading()` is true, and reads the texture's own after `waitForLoads` and the main-thread publish; a failed texture reads the white fallback. On the null device, in `assets_tests`.
...
Found while deriving the normal matrix in the shader ([rendering.md](rendering.md#gpu-driven-submission)), and older than that change. `world` allows a negative scale — the inspector takes one, and `Transform::fromMatrix` decomposes a mirrored basis into one — but every pipeline is built with a counter-clockwise front face, fixed at creation, and nothing flips it per draw. A mirroring transform reverses a triangle's winding on screen, so the rasterizer takes a mirrored draw's outside for its back: a single-sided one is culled inside out, showing its far faces through its near ones, in the shadow, depth, id and selection passes as well. A double-sided one draws the right faces and, since the normal is now the cofactor matrix's, shades correctly, because the cofactor's sign and `SV_IsFrontFace` both flip and cancel.

The steps:

1. `rhi`: the front face as dynamic state, `vk::DynamicState::eFrontFace`, core since Vulkan 1.3 and within the extended dynamic state the baseline already relies on ([Vulkan baseline](rendering.md#vulkan-baseline)); `ICommandList::setFrontFace`, traced by the null device as `setFrontFace Clockwise`; pipelines default it to counter-clockwise.
2. `renderer`: a draw is mirrored when its transform's upper 3×3 has a negative determinant, worked out where `prepareFrame` already transforms the bounds. Mirrored draws sort into batches of their own, after double-sidedness and before the mesh, and each batch sets its front face before its indirect call; the blended draws, recorded one by one, set it when it changes. Every drawing pass shares the batches, so the shadow, depth, id and selection passes follow.
3. `sonnet.slang`: once the winding is corrected, `SV_IsFrontFace` no longer flips for a mirrored draw, so the cofactor's sign no longer cancels and `transformNormal` would point a mirrored draw's normals inward. It multiplies by the sign of the determinant, `dot(cross(x, y), z)`, which it has the vectors for, and its comment says why.
4. The test, on Lavapipe: a single-sided box scaled by (-1, 1, 1) renders the same pixels as the unscaled box, which a box's symmetry makes exact, and fails today; the null-device test asserts a mirrored draw's batch is preceded by `setFrontFace Clockwise`.
