...
4. PR [#28](https://github.com/Pacheco95/sonnet/pull/28) is superseded by this fix; its Linux setup section is retained in [build.md](build.md#linux-setup). The manifest has no Vulkan loader window-system features. `/setup-sonnet` no longer suggests the `LD_LIBRARY_PATH` workaround.
