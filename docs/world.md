...
The phases `Input`, `FixedUpdate`, `Update`, `PostUpdate` and `PreRender` are flecs phase entities in a dependency chain; systems name theirs with `kind(world.phase(...))`. Systems marked with `addToSimulation` (the spin system and physics) belong to play mode; the transform system runs always.
